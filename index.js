    ...
    if(key&&num&&user_info.number&&user_info.user&&user_info.code&&user_info.pass){
      cb.getValidateStatus(key, (error, response) => {
        if(error)socket.emit('verify_loi','A5');
          ...
          else socket.emit('verify_loi','A6');
        }
      });
 ...
 function get_time(gio){
    ...
    if(key&&num&&id&&pass){
      cb.getValidateStatus(key, (error, response) => {
        if(error)socket.emit('S_doipass_thatbai','A1');
          ...
          if(response.number==num && response.validated)
          {
            con.query("UPDATE `account` SET `pass` = '"+passwordHash.generate(pass)+"' WHERE `number` LIKE '"+num+"'",function(err){
              if(err)socket.emit('S_doipass_thatbai','A2');
              else socket.emit('S_doipass_thanhcong');
            });
          }
          else socket.emit('S_doipass_thatbai','A3');
        }
      });
  }
  });
  ...
  socket.on('search_contact', function (string){
    if (socket.number&&string){
      console.log(string);
      con.query("SELECT `number`, `user` FROM `account` WHERE `number` LIKE CONCAT('%',"+string+",'%')", function(err, a1){
      if(err)console.log(err);
      else
      {
        if(a1.length>0){
