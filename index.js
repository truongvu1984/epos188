 ...
 function get_time(gio){
                    ...
                    con.query("SELECT * FROM `list_err` WHERE id > "+tin.tt+" ORDER BY id ASC", function(err1, row1s){
                      if (err1){console.log(err1);}
                        ...
                        row1s.forEach((row1, i) => {
                          ...
                          socket.emit("S_send_nhiemvu",{tt:row1.id,idc:row1.idc,ten:row1.ten,mota:row1.mota,giaonv1:get_time(row1.giaonv1),ch1_hoten:row1.ch1_hoten,ch1_chucvu:row1.ch1_chucvu,ch1_donvi:row1.ch1_donvi,
                    ...
                    if(tin.bantin!=undefined&&tin.bantin.length>0){
                      ...
                      con.query("SELECT * FROM `list_err` WHERE `idc` LIKE '"+item.idc+"' AND `"+item.cot+"`NOT LIKE '' LIMIT 1", function(err2, row2s){
                          if (err2){console.log(err2);}
                            ...
                            socket.emit("S_capnhat",{idc:item.idc,giaonv2:row2s[0].giaonv2,ch2_hoten:row2s[0].ch2_hoten,ch2_chucvu:row2s[0].ch2_chucvu,ch2_donvi:row2s[0].ch2_donvi,batdau:batdau,dennoi:dennoi,xong:xong,vedonvi:vedonvi,a0:a0,a1:a1,a2:a2,a3:a3,a4:a4});
                          }
                        });
                      });
                    }
                    ...
                    if(tin.bantin.length>0){
                      ...
                      con.query(lenh, function(err2, row2s){
                          if (err2){console.log('4444'+err2);}
                            ...
                            let xong=null;if(row2s[0].xong!=null){xong=get_time(row2s[0].xong);a3='A';}else a3='B';
                            let vedonvi=null;if(row2s[0].vedonvi!=null){vedonvi=get_time(row2s[0].vedonvi);a4='A';}else a4='B';
                          socket.emit("S_capnhat",{idc:item.idc,giaonv2:row2s[0].giaonv2,ch2_hoten:row2s[0].ch2_hoten,ch2_chucvu:row2s[0].ch2_chucvu,ch2_donvi:row2s[0].ch2_donvi,
  ...
  socket.on('giao_nhiemvu', function(tin){
       if (socket.user!=null && socket.type!=null&&socket.type=="B"){
         var idc = 'r'+Date.now();
         let thoigian=new Date();
         var sql = "INSERT INTO `list_err` (idc,ten, mota,giaonv1,ch1_user,ch1_hoten,ch1_chucvu,ch1_donvi) VALUES ?";
           var values = [[idc,tin.ten,tin.mota,thoigian,tin.user,tin.hoten,tin.chucvu,tin.donvi]];
           con.query(sql, [values], function (err1, result) {
             if (err1)socket.emit("giao_nhiemvu_thatbai","A");
             else {
                   socket.emit("giao_nhiemvu_ok",{tt:result.insertId ,idc:idc,time:get_time(thoigian)});
                   io.sockets.in("chung").emit("S_send_nhiemvu",{tt:result.insertId,idc:idc,ten:tin.ten,mota:tin.mota,giaonv1:get_time(thoigian),
                   ch1_hoten:tin.hoten,ch1_chucvu:tin.chucvu,ch1_donvi:tin.donvi,a0:'B',a1:'B',a2:'B',a3:'B',a4:'B'});
                   io.sockets.in(tin.user).emit("S_send_nhiemvu",{tt:result.insertId,idc:idc,ten:tin.ten,mota:tin.mota,giaonv1:get_time(thoigian),
                   ch1_hoten:tin.hoten,ch1_chucvu:tin.chucvu,ch1_donvi:tin.donvi,a0:'B',a1:'B',a2:'B',a3:'B',a4:'B'});
             }
           });
        }
